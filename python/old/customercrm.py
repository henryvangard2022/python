...
mydb = mysql.connector.connect(
    ...
    user="root",
    password="2597",
    database="customercrm"  # this is the database to use
)
...
#
# The fields and values of the table 'customer'
#
customerValues = {
    'customer_id': '',
    'first_name': '',
...
def EnterCustomerID():
    id = ''
    while id == '':
        print('Please enter a value and NOT just press the Enter key!')
...
def SearchCustomer():
    ...
    print('4. Search ALL\n')
    print('100. Return to Customer CRM - Main Menu\n')
    choice = input('Enter a choice: ')

    if int(choice) == 100:  # return to main menu
        return
...
def UpdateCustomer():  # by customer ID only
    ...
    # then I can prepend the ', ' the latter fields
    firstVal = True

    print('>>>>>>>>>>>>>>>>>>>>>>>>>>>> Update Menu <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n')

    # this value is mandatory and cannot be blank!!!
    customerID = EnterCustomerID()

    print('\nEnter a value or press Enter to leave it blank.\n')

    #############################################################
    # START building the UPDATE command with the entered values #
    #############################################################

    firstName = input('First Name: ')
    if firstName != '':
        updateSQL = updateSQL + 'first_name = %s '  # concat to UPDATE command
    ...
    ###############################################################################################
    # FINISH the UPDATE command with the WHERE clause and the tuple of values entered by the user #
    ###############################################################################################

    updateSQL = updateSQL + whereStr  # append the WHERE clause
    valStr.append(customerID)  # add the customer ID to the values

    # valStr is a list but it does NOT matter, execte () still works!!!
